...
Since version 1.3, each custom organism is stored in its own file under `inst/extdata/organisms`, next to a small index in `data/organisms_index.rda` that DIANE reads at startup to list them.

The scripts that produce them are available on request at alexandre.soriano@cirad.fr. They take a yaml file, with path to the different files needed to store organisms and datasets informations as well as some descriptive informations. All those files are tab separated.
